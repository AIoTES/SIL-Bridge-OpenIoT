...
public class OpenIoTTranslator extends SyntacticTranslator<String> {
    ...
    public OpenIoTTranslator() {
        super(openiotbaseURI, "OPENIOT");
    ...
    public Model toJenaModel(String formatXString) throws IOException {
        ...
        if (topLevelNode.isObject()) {
            Resource myEntity = jenaModel.createResource(openiotbaseURI+System.currentTimeMillis());
    ...
    private void parseJSONObjectToJena(Resource objectResource, JsonNode objectNode, Model jenaModel) {
        ...
        while (it.hasNext()) {
            ...
            if (field.getKey().equals("id")) {
              ...
              //  String id = field.getValue().asText();//field.getKey() ;//idNode.get("id").asText();
            ...
            /*    String ontName = null;
                while(it2.hasNext()){
                	...
                	if(node.isObject()) {
                		if(!field2.getKey().equals("contextData") && !field2.getKey().equals("id")){ 
                			ontName =  field2.getKey();
                			break;
                		}
                	}
                }
               if(ontName != null) objectResource.addProperty(hasId, eu.interiot.intermw.bridge.openiot.OpenIoTUtils.urlize(id, ontName));
            	...
            	if(node.isObject()){
            		// The returned JSON object for an ontology instance has id and ContextData fields
            			if(objectNode.has("id") && objectNode.has("contextData")){	
...
//            System.out.println(resultNode);
            ...
            if (
                //Assume that a resource that has some RDF type from OPENIOT is a OPENIOT object/array/value
                    types.contains(arrayType) ||
                            types.contains(valueType) ||
                            types.contains(instanceType) ||
                            //Assume that a resource that has some of the OPENIOT properties is of OPENIOT type
        ...
        if (jsonNodeList.isEmpty()) {
            ...
            throw new IllegalSyntaxException("No top-level RDF entity found (a root node in RDF graph is required in OPENIOT RDF)");
...
//                typesIt.remove();
    ...
    private void parseAttributesToJson(Resource entityResource, ObjectNode entity, Model jenaModel, ObjectMapper mapper) {
        ...
        //Parse id
        NodeIterator nodeIterator = jenaModel.listObjectsOfProperty(entityResource, hasId); // TRANSLATE AS {"id":{"idid":"..."}}
        if (nodeIterator.hasNext()) {
        	value = nodeIterator.next().toString();
            ObjectNode attributeNode = mapper.createObjectNode();
//            attributeNode.put("idid", value);
            attributeNode.put("idid", eu.interiot.intermw.bridge.openiot.OpenIoTUtils.deurlize(value)); // Remove URI format
            entity.set("id", attributeNode);
        }
